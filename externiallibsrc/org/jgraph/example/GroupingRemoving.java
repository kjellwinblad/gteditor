...
public class GroupingRemoving {
	...
	public static void main(String[] args) {
		...
		DefaultGraphCell group = createVertex("Hello", 20, 20, 40, 20, null,
				false);
		...
		graph.getGraphLayoutCache().insert(new Object[] { group }, null, null,
				pm, null);

		// Set some of the children are hidden
		graph.getGraphLayoutCache().hideCells(
				new Object[] { cells[3], cells[7] }, true);
		...
		model.remove(DefaultGraphModel.getDescendants(model,
				DefaultGraphModel.getRoots(model)).toArray());
		System.out.println("model.roots.size() = "
				+ DefaultGraphModel.getRoots(model).length);
		System.out.println("cell views size = " + cache.getCellViews().length);
		System.out.println("hidden views size = "
				+ cache.getHiddenCellViews().length);
		System.out
				.println("visible set size = " + cache.getVisibleSet().size());
