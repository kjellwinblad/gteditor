...
public class GraphTransferHandler extends TransferHandler {
	 ...
	 * 
	 * @param graph
	 *            the graph the insertion is occurring on
	 * @param inCount
	 *            the number of time the insert has been applied
	 * @param bounds
	 *            the bounds of the transferred graph
	 * @return the offset from the cloned cell(s)
	 */
	protected Point2D getInsertionOffset(JGraph graph, int inCount,
			Rectangle2D bounds) {
		Point2D result = null;
		if (graph != null) {
			result = new Point2D.Double(inCount * graph.getGridSize(), inCount
					* graph.getGridSize());
		}
		return result;
	}
