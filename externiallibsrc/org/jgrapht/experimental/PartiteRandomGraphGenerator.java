 ...
 * Contributor(s):   -
 *
 * $Id: PartiteRandomGraphGenerator.java,v 1.2 2007/05/22 15:52:59 kjellw Exp $
 *
 * Changes
 * -------
 ...
 * @since Sep 13, 2004
 */
public class PartiteRandomGraphGenerator<V, E> implements
	 ...
	 * 
	 * @param numVertices
	 *            number of vertices in the k partitions
	 * @param numEdges
	 *            number of edges to be generated between any two partitions
	 * 
	 * @throws IllegalArgumentException
	 */
	 ...
	 * @param vertexFactory
	 * @param resultMap
	 *            some array of vertices
	 * 
	 * @see GraphGenerator#generateGraph
	 */
	public void generateGraph(Graph<V, E> target,
			VertexFactory<V> vertexFactory, Map<String, Object[]> resultMap) {
		Object[][] vertices = new Object[numVertices.length][];

		for (int i = 0; i < numVertices.length; i++) {
			vertices[i] = RandomGraphHelper.addVertices(target, vertexFactory,
					numVertices[i]);
			...
			for (int j = 0; j < i; j++) {
				RandomGraphHelper.addEdges(target, Arrays.asList(vertices[i]),
						Arrays.asList(vertices[j]), numEdges);
			}
		}
	}
}
