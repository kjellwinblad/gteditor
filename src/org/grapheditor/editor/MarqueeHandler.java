...
import org.grapheditor.editor.menus.NothingSelectedMenu;
import org.grapheditor.editor.menus.OnlyEdgesSelectedMenu;
import org.grapheditor.editor.menus.VertexSelectedMenu;
...
public class MarqueeHandler extends BasicMarqueeHandler {
	...
	private VertexSelectedMenu vertexSelectedMenu;

	private OnlyEdgesSelectedMenu onlyEdgeSelectedMenu;

	private NothingSelectedMenu nothingSelectedMenu;
	...
	public MarqueeHandler(GraphEditorPane graph) {
		super();
		this.graph = graph;
		vertexSelectedMenu = new VertexSelectedMenu(graph);
		onlyEdgeSelectedMenu = new OnlyEdgesSelectedMenu(graph);
		nothingSelectedMenu = new NothingSelectedMenu(graph);
	}
	...
	private void displayPopUpMenu(final Point point) {
			...
			public void run() {
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						Object[] selectedEdges = graph.getSelectionCells(graph
								.getGraphLayoutCache().getCells(false, false,
										false, true));

						if (selectedVerteces.length != 0) {
							// 1)
							vertexSelectedMenu
									.setEdgesSelected(selectedEdges.length != 0);
							vertexSelectedMenu.show(graph, (int) point.getX(),
									(int) point.getY());
						} else if (selectedEdges.length != 0) {
							// 2)
							onlyEdgeSelectedMenu.show(graph,
									(int) point.getX(), (int) point.getY());
						} else {
							nothingSelectedMenu.show(graph, (int) point.getX(),
									(int) point.getY());
						}
		...
		// Object cell = graph.getFirstCellForLocation(point.getX(),
		// point.getY());
