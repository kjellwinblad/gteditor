...
/**
 * This class has a method that displayes an interface that the user can use to
 * select a file to export a neigbour list to.
 * 
 * @author kjellw
 * 
 */
public class NeighbourListExporterVisual {

	public static void doNeighborListExport(final JFrame parent,
			final GraphEditorPane editorPane) {
		final JFileChooser fileChooser = new JFileChooser();
		fileChooser.setDialogTitle("Select file to export to...");
		int option = fileChooser.showSaveDialog(parent);
		if (option == JFileChooser.APPROVE_OPTION) {
			final NeighbourListExporter neiExp = new NeighbourListExporter();
			new CancelAbleProcessDialog(
					parent,
					new CancelAbleProcess() {

						public void setStatusNotifier(StatusNotifier n) {
							neiExp.setStatusNotifier(n);

						}

						public void run() {
							try {
								neiExp.export(fileChooser.getSelectedFile(),
										editorPane);
							} catch (IOException e) {
