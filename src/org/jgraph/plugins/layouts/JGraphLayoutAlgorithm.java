...
public abstract class JGraphLayoutAlgorithm {
	...
	/**
	 * Subclassers may return a new JComponent that allows to configure the
	 * layout. The default implementation returns <code>null</code>.<br>
	 * Note: Settings creation may be expensive so the UI should cache the
	 ...
	 * 
	 * @param graph
	 *            JGraph to be altered by layout
	 * @param cells
	 *            Array of cells to be moved by the layout
	 */
	...
	/**
	 * Executes the layout algorithm specifying which cells are to remain in
	 * place after the layout is applied.
	 * 
	 * @param jgraph
	 *            JGraph to be altered by layout
	 * @param dynamic_cells
	 *            Cells that are to be moved by the layout
	 * @param static_cells
	 *            Cells that are not to be moved, but allowed for by the layout
	 */
	public abstract void run(JGraph jgraph, Object[] dynamic_cells,
			Object[] static_cells);
	...
	public static JDialog createDialog(final JGraphLayoutSettings settings,
		...
		if (settings instanceof Component)
			return populateDialog(settings, new JDialog(parent, title, true),
					close, apply);
		return null;
	}
	...
	public static JDialog populateDialog(final JGraphLayoutSettings settings,
		...
		if (dialog != null && settings instanceof Component) {
			...
			dialog.getContentPane().add((Component) settings,
					BorderLayout.CENTER);
	 ...
	 * 
	 * @param sourceGraph
	 *            The JGraph to have the layout applied
	 * @param layout
	 *            The layout algorithm to be applied
	 * @param cells
	 *            Cells that are to be moved by the layout
	 */
	public static void applyLayout(JGraph sourceGraph,
			JGraphLayoutAlgorithm layout, Object[] cells) {
	 ...
	 * specified layout algorithm on the local JGraph. After the layout has been
	 * applied, the changed attributes on the local JGraph are extracted and
	 * applied to the JGraph instance passed in, creating one undoable edit.
	 * 
	 * @param sourceGraph
	 *            The JGraph to have the layout applied
	 * @param layout
	 ...
	 */
	public static void applyLayout(JGraph sourceGraph,
			JGraphLayoutAlgorithm layout, Object[] dynamic_cells,
			Object[] static_cells) {
