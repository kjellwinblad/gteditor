...
public class CircleGraphLayout extends JGraphLayoutAlgorithm {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see java.lang.Runnable#run() @param graph JGraph instance @param
	 *      dynamic_cells List of all nodes the layout should move @param
	...
	public void run(JGraph graph, Object[] dynamic_cells, Object[] static_cells) {
		...
		for (int i = 0; i < views.length; i++) {
			...
			if (views[i] instanceof VertexView) {
				...
				// Update Maximum
				if (bounds != null)
					max = (int) Math.max(max, Math.max(bounds.getWidth(),
							bounds.getHeight()));
			}
		}
		// Compute Radius
		...
		for (int i = 0; i < vertices.size(); i++) {
			...
			// Update Location
			if (bounds != null)
				bounds.setFrame(r + r * Math.sin(i * phi), r + r
						* Math.cos(i * phi), bounds.getWidth(), bounds
						.getHeight());
		}
	}
